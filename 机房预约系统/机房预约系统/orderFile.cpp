...
OrderFile::OrderFile()
{
	...
	while (ifs >> date && ifs >> interval && ifs >> stuId && ifs >> stuName && ifs >> roomId && ifs >> status)
	{
		...
		if (pos != -1)
		{
			key = date.substr(0, pos);//从0号开始，截取pos个
...
void OrderFile::updateOrder()
{
	...
	ofstream ofs(ORDER_FILE, ios::out | ios::trunc);//清除并重写
	for (int i = 0; i < this->m_Size; i++)
	{
		ofs << "date:" << this->m_orderData[i]["date"] << " ";//this->m_orderData[KEY][key] 获取key对应的value
